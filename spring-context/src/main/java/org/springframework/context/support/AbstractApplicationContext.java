...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//该后置处理器会调用一系列aware接口的回调方法,较重要的ApplicationContextAware用于获取使用容器上下限文
		 ...
		 *
		 * 该方法有一个作用，指定接口被注入时使用哪个实现类，如果不指定当接口有多个实现类时，IOC容器不能确定注入哪个bean，会报错
		 */
		...
		//注册了一个事件监听器探测器后置处理器接口，用于向多播器中添加事件（基于接口类型的事件，非注解形式的）
	...
	protected void registerListeners() {
		...
		//获取bean定义中的监听器对象,此种方式是为了防止标注懒加载的事件监听器没有被后置处理器注册到多播器上，仅限接口类型的事件监听器，调用此方法后在调用后置处理器，后置处理器添加的为完整的bean，从而提高性能
		String[] listenerBeanNames = getBeanNamesForType(ApplicationListener.class, true, false);
		//把监听器的名称注册到我们的多播器上，注册的只是名称，调用的时候需要调用getBean获取实例
